...
export function useFirestoreSync() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // Keep default items and append firestore items
      useAppStore.setState(state => {
        const defaultItems = state.knowledgeBase.filter(i => i.id.startsWith('default-'));
        return { knowledgeBase: [...defaultItems, ...items] };
      });
    }, (error) => {
      handleFirestoreError(error, OperationType.LIST, path);
    });
