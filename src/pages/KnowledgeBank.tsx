...
export function KnowledgeBank() {
  const { knowledgeBase, addKnowledgeItem, deleteKnowledgeItem, activeProjectId, projects } = useAppStore();
        ...
        {filteredItems.map(item => (
          <div key={item.id} className="bg-[#161616] border border-white/5 rounded-sm p-4 relative group">
             <div className="absolute top-4 right-4 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
               <button onClick={() => deleteKnowledgeItem(item.id)} className="text-red-500/50 hover:text-red-500 bg-red-500/10 p-2 md:p-1.5 rounded-sm flex items-center justify-center min-h-[44px] min-w-[44px] md:min-h-0 md:min-w-0">
